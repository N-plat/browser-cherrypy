...
def redirect_if_authentication_is_required_and_session_is_not_authenticated(*args, **kwargs):
    ...
    if conditions is not None:
        ...
        if not username:
            ...
            raise cherrypy.HTTPRedirect("/auth/login?from_page=%22"+from_url.replace('&&','%01').replace('%22','%00')+"%22")
...
#    cherrypy.config.update({'server.socket_port': 80})
    cherrypy.config.update({'server.socket_host': 'ec2-18-236-80-185.us-west-2.compute.amazonaws.com'})
